...
const moodWords = ["electric", "digital", "neon", "synthetic", "futuristic", "cyber"]

// Helper to get an icon based on persona name
const getPersonaIcon = (personaName: string): JSX.Element => {
  if (personaName.includes("Online")) return <Laptop className="h-4 w-4 text-cyan-400" />;
  if (personaName.includes("Country")) return <Truck className="h-4 w-4 text-cyan-400" />;
  if (personaName.includes("Rock")) return <Guitar className="h-4 w-4 text-cyan-400" />;
...
export default function AltPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-cyan-900/20 to-black relative overflow-hidden">
      {/* Animated Tron Background */}
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute inset-0">
          <div className="absolute w-full h-px bg-gradient-to-r from-transparent via-cyan-400 to-transparent opacity-30 animate-pulse" 
          ...
          <div className="absolute w-full h-px bg-gradient-to-r from-transparent via-blue-400 to-transparent opacity-20 animate-pulse" 
               style={{ top: '60%', animationDuration: '6s', animationDelay: '2s' }}></div>
        </div>
      </div>
      ...
      {showPersonas && (
        ...
        >
          <motion.div
            ...
            className="bg-slate-900/90 backdrop-blur-md border border-cyan-500/30 rounded-2xl p-8 max-w-6xl w-full max-h-[80vh] overflow-y-auto shadow-2xl shadow-cyan-500/20"
          ...
          >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                {featuredPersonas.map((persona) => (
                  <motion.button
                    ...
                    className={`text-left p-6 bg-slate-800/50 backdrop-blur-md border border-cyan-500/30 rounded-xl hover:bg-slate-700/50 hover:border-cyan-400/50 transition-all group ${selectedPersonaId === persona.id ? 'ring-2 ring-cyan-500 shadow-lg shadow-cyan-500/20' : ''}`}
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <Avatar className="h-16 w-16 border-2 border-cyan-500">
                        ...
                        <AvatarFallback className="bg-slate-700">{persona.name.substring(0, 2)}</AvatarFallback>
                      </Avatar>
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {persona.artists.slice(0, 3).map((artist) => (
                            <span key={artist} className="text-xs bg-cyan-500/20 text-cyan-300 px-2 py-1 rounded-full border border-cyan-500/30">
                              {artist}
                            </span>
                          ))}
                        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {otherPersonas.map((persona) => (
                  <motion.button
                    key={persona.id}
                    onClick={() => selectPersona(persona)}
                    className={`text-left p-4 bg-slate-800/50 backdrop-blur-md border border-cyan-500/30 rounded-xl hover:bg-slate-700/50 hover:border-cyan-400/50 transition-all group ${selectedPersonaId === persona.id ? 'ring-2 ring-cyan-500 shadow-lg shadow-cyan-500/20' : ''}`}
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
            ...
            <div className="flex justify-center mt-8">
              <button
                onClick={() => setShowPersonas(false)}
                className="px-6 py-3 bg-slate-800/50 backdrop-blur-md border border-cyan-500/30 rounded-xl text-white hover:bg-slate-700/50 hover:border-cyan-400/50 transition-all"
              >
                Close
              </button>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative z-20 text-center mb-16">
            <motion.div
              ...
              className="mb-8"
            >
              <h1 className="text-6xl md:text-8xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-blue-500 to-teal-500 mb-6 font-mono tracking-wider">
                NEON.BEATS
              </h1>
              <div className="text-2xl md:text-4xl text-white font-light font-mono">
            ...
            <motion.p
              ...
              className="text-lg md:text-xl text-gray-300 max-w-4xl mx-auto mb-12 font-mono"
            >
              AI-powered music discovery with personalized personas that understand your digital emotions and find the perfect soundtrack for every cyber moment.
            </motion.p>
            ...
            >
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 rounded-full transition-all bg-slate-800/50 backdrop-blur-md border border-cyan-500/30 text-gray-300 hover:bg-slate-700/50 hover:border-cyan-400/50 hover:shadow-lg hover:shadow-cyan-500/20"
              >
            ...
            {selectedPersona && (
              <motion.div
                ...
                className="mb-8"
              >
                <div className="inline-flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 backdrop-blur-md border border-cyan-500/30 rounded-2xl shadow-lg shadow-cyan-500/20">
                  <Avatar className="h-8 w-8 border-2 border-cyan-400">
                    ...
                    <AvatarFallback className="bg-cyan-600 text-white text-sm">{selectedPersona.name.substring(0, 2)}</AvatarFallback>
                  </Avatar>
            ...
            >
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                <div className="relative flex-grow">
                  <textarea
                    className="w-full px-6 py-4 bg-slate-800/50 backdrop-blur-md border border-cyan-500/30 rounded-2xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent text-lg h-24 resize-none font-mono"
                ...
                <div className="flex items-center space-x-2 flex-shrink-0 self-center sm:self-auto">
                  ...
                  />
                  <Label htmlFor="exclude-mainstream-toggle" className="text-sm text-white whitespace-nowrap font-mono">
                    Avoid Hits
                  </Label>
                </div>
                <Button
                  ...
                  className="px-6 py-4 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-2xl text-white font-medium hover:from-cyan-600 hover:to-blue-600 transition-all duration-200 flex items-center gap-2 h-12 flex-shrink-0 w-full sm:w-auto shadow-lg shadow-cyan-500/20 font-mono"
                >
                  {loading ? (
